...
llmgate stores OAuth credentials for your personal LLM subscriptions. They are
sensitive bearer secrets.

- **At rest:** the auth store (`~/.llmgate/auth.json`) is written with mode `0600`
...
- **No telemetry:** llmgate makes no outbound calls other than to provider OAuth
  and inference endpoints.

## Scope and intended use

llmgate is for personal, single-user use of your own subscriptions. See the
