export const DEFAULT_CONFIG_YAML = `# llmgate configuration
...
server:
  ...
  port: 8787
  # api_key: \${LLMGATE_API_KEY}  # client auth (timing-safe); REQUIRED when host is not loopback
...
models:
  ...
  grok:
    provider: xai
    upstream: grok-build      # Grok Build model names churn; verify with 'llmgate models --remote'
